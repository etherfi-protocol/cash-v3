...
contract TopUpSourceSetEURCMainnet is Utils, GnosisHelpers, Test {
    ...
    function run() public {
        ...
        string memory path = string.concat("./output/TopUpEURCSetConfig-", chainId, ".json");
    ...
    function parseTokenConfigs(string memory jsonString, string memory chainId) internal view returns (address[] memory tokens, TopUpFactory.TokenConfig[] memory tokenConfig) {
        ...
        for (uint256 i = 0; i < 20; i++) {
            ...
            if (keccak256(bytes(tokenName)) == keccak256(bytes("EURC"))) {
                usdtIndex = i;
                found = true;
                break;
            }
        ...
        if (!found) revert("EURC token config not found");
