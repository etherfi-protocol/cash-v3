...
contract UpgradeSettlementDispatcherV2OP is Utils, GnosisHelpers {
    ...
    function run() public {
        ...
        // Settlement recipients for USDC + USDT (all 4 dispatchers)
        txs = _addSettlementRecipientTransactions(txs, REAP_PROXY, RAIN_PROXY, PIX_PROXY, CARD_ORDER_PROXY);
    ...
    function _addSettlementRecipientTransactions(
        ...
        txs = string(abi.encodePacked(txs, _getGnosisTransaction(addressToHex(pixProxy), data, "0", false)));
        txs = string(abi.encodePacked(txs, _getGnosisTransaction(addressToHex(cardOrderProxy), data, "0", true)));
