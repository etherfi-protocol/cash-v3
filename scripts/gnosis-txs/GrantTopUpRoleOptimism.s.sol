...
/// @title GrantTopUpRoleOptimism (Gnosis)
/// @notice Generates a Gnosis Safe batch to grant TOP_UP_ROLE to addresses on OP mainnet.
///
/// Usage:
///   forge script scripts/gnosis-txs/GrantTopUpRoleOptimism.s.sol:GrantTopUpRoleOptimism --rpc-url <RPC> --broadcast
contract GrantTopUpRoleOptimism is GnosisHelpers, Utils {
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < wallets.length; ++i) {
            ...
            string memory grantRole = iToHex(abi.encodeWithSelector(IRoleRegistry.grantRole.selector, TOP_UP_ROLE, wallets[i]));
            txs = string(abi.encodePacked(txs, _getGnosisTransaction(addressToHex(roleRegistry), grantRole, "0", isLast)));
        }

        vm.createDir("./output", true);
        string memory path = string.concat("./output/GrantTopUpRole-", chainId, ".json");
        vm.writeFile(path, txs);

        vm.stopBroadcast();

        console.log("Generated gnosis tx granting TOP_UP_ROLE to %s wallets on chain %s", wallets.length, chainId);
