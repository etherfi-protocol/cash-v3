...
/**
 * @title DeployMigrationBridgeModule
 * @notice Deploys and fully configures the MigrationBridgeModule on Scroll for dev.
 *         Uses CREATE3 for deterministic addresses so VerifyMigrationBridgeModule can verify.
 *
 * Usage:
 *   ENV=dev forge script scripts/migration/DeployMigrationBridgeModule.s.sol \
 *     --rpc-url $SCROLL_RPC --broadcast
 */
contract DeployMigrationBridgeModule is Utils {
    address constant NICKS_FACTORY = 0x4e59b44847b379578588920cA78FbF26c0B4956C;

    // Deterministic salts (dev)
    bytes32 constant SALT_MIGRATION_MODULE_IMPL  = keccak256("MigrationBridgeModule.Dev.Impl");
    bytes32 constant SALT_MIGRATION_MODULE_PROXY = keccak256("MigrationBridgeModule.Dev.Proxy");
    bytes32 constant SALT_HOOK_IMPL              = keccak256("MigrationBridgeModule.Dev.HookImpl");
    ...
    function run() public {
        ...
        EtherFiDataProvider dataProvider = EtherFiDataProvider(dataProviderAddr);
        EtherFiHook hook = EtherFiHook(hookAddr);
        RoleRegistry roleRegistry = RoleRegistry(roleRegistryAddr);

        vm.startBroadcast(deployerPrivateKey);

        // ── 1. Deploy impl via CREATE3 ──
        ...
        // ── 2. Deploy proxy via CREATE3 (atomic init) ──
        ...
        // ── 4. Deploy new hook impl + upgrade + set migration bypass ──
        ...
        console.log("Hook impl:", newHookImpl);

        // ── 5. Grant admin role ──
        roleRegistry.grantRole(migrationModule.MIGRATION_BRIDGE_ADMIN_ROLE(), deployer);

        // ── 6. Configure token bridge routes ──
        _configureTokens(migrationModule);
    ...
    function _configureTokens(MigrationBridgeModule module) internal {
        address[] memory tokens = new address[](17);
        MigrationBridgeModule.TokenBridgeConfig[] memory configs = new MigrationBridgeModule.TokenBridgeConfig[](17);
        ...
        module.configureTokens(tokens, configs);
    }
}
