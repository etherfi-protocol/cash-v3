 ...
 *         Raw Chainlink aggregators already satisfy that surface (latestAnswer/decimals/description)
 *         and are used as-is, Aave-style (staleness is monitored off-chain, not enforced on-chain):
 *         - Direct Chainlink aggregators: USDT, WETH (ETH/USD), OP, and frxUSD at USDC/USD
 *           ($1-stable), matching the DebtManager's stable treatment.
 ...
 *           the teller) x an underlying IAaveV4PriceFeed: liquidETH/liquidBTC/liquidUSD/eBTC/eUSD
 *           over raw Chainlink legs, and sETHFI (rate in ETHFI) over the ETHFI/USD PythPriceFeed.
 *         - Midas receipt tokens via ChainlinkCompositePriceFeed over their 8-dec latestRoundData
 *           proxies: weEUR (weEUR/EUR x EUR/USD), liquidRESERVE and liquidRWA (x USDC/USD $1 leg).
 *
...
contract AddSummerLendCollateral is Utils {
    ...
    function run() public {
        ...
        _list(LIQUID_ETH, _veda(LIQUID_ETH_TELLER, ETH_USD, "liquidETH / USD"), 50_00, 10_500);
        _list(LIQUID_BTC, _veda(LIQUID_BTC_TELLER, BTC_USD, "liquidBTC / USD"), 50_00, 10_500);
        _list(LIQUID_USD, _veda(LIQUID_USD_TELLER, USDC_USD, "liquidUSD / USD"), 80_00, 10_200);
        _list(EBTC, _veda(EBTC_TELLER, BTC_USD, "eBTC / USD"), 52_00, 10_500);
        _list(EUSD, _veda(EUSD_TELLER, USDC_USD, "eUSD / USD"), 80_00, 10_200);

        // Midas receipt tokens (proxy rate x raw Chainlink USD leg)
        _list(LIQUID_RESERVE, _composite(LIQUID_RESERVE_USD_PROXY, USDC_USD, "liquidRESERVE / USD"), 80_00, 10_100);
        _list(WEEUR, _composite(WEEUR_EUR_PROXY, EUR_USD, "weEUR / USD"), 70_00, 10_200);
        _list(LIQUID_RWA, _composite(LIQUID_RWA_USD_PROXY, USDC_USD, "liquidRWA / USD"), 70_00, 10_400);

        // Direct listings on raw Chainlink aggregators (they satisfy the Aave feed surface natively)
        _list(USDT, _direct(USDT_USD, "USDT / USD"), 90_00, 10_100);
        _list(WETH, _direct(ETH_USD, "ETH / USD"), 55_00, 10_350);
        _list(OP, _direct(OP_USD, "OP / USD"), 20_00, 10_500);
        // frxUSD: $1-stable like the DebtManager treats it; USDC/USD is the closest live $1 feed
        _list(FRXUSD, _direct(USDC_USD, "USDC / USD"), 90_00, 10_100);
    ...
    /// @dev A raw Chainlink USD aggregator used as-is, sanity-checked
    function _direct(address aggregator, string memory desc) internal view returns (address) {
        _requireLivePrice(aggregator, desc);
        return aggregator;
    }
    ...
    /// @dev Deploys a ChainlinkCompositePriceFeed over a Midas rate proxy and a USD leg
    function _composite(address rateProxy, address usdLeg, string memory desc) internal returns (address) {
        ChainlinkCompositePriceFeed feed = new ChainlinkCompositePriceFeed(IAggregatorV3(rateProxy), IAggregatorV3(usdLeg), FEED_DECIMALS, MIDAS_RATE_MAX_STALENESS, CHAINLINK_MAX_STALENESS, desc);
        _requireLivePrice(address(feed), desc);
        return address(feed);
    }
