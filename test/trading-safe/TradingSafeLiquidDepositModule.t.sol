...
contract TradingSafeLiquidDepositModuleTest is TradingSafeTestBase {
    ...
    function setUp() public {
        ...
        module = new TradingSafeLiquidDepositModule(_single(address(liquidBtc)), _single(address(teller)), address(dataProvider));
        roleRegistry.grantRole(module.TRADING_SAFE_LIQUID_DEPOSIT_MODULE_ADMIN(), owner);
    ...
    function test_constructor_revertsOnZeroConfig() public {
        address[] memory liquidAssets = _single(address(0));
        vm.expectRevert(ITradingSafeLiquidDepositModule.InvalidConfiguration.selector);
        new TradingSafeLiquidDepositModule(liquidAssets, _single(address(teller)), address(dataProvider));
    }

    function test_constructor_revertsIfTellerVaultMismatch() public {
        MockBoringVault otherVault = new MockBoringVault();
        vm.expectRevert(ITradingSafeLiquidDepositModule.InvalidConfiguration.selector);
        new TradingSafeLiquidDepositModule(_single(address(otherVault)), _single(address(teller)), address(dataProvider));
    }

    function test_constructor_revertsIfConfigArrayLengthsMismatch() public {
        vm.expectRevert(ITradingSafeLiquidDepositModule.ConfigArrayLengthMismatch.selector);
        new TradingSafeLiquidDepositModule(_single(address(liquidBtc)), new address[](0), address(dataProvider));
    }

    // ---- Route configuration ----

    function test_routeAdmin_addsFutureVaultAndDepositsDifferentAsset() public {
        MockERC20 otherAsset = new MockERC20("Other Asset", "OTHER", 6);
        MockBoringVault otherLiquidAsset = new MockBoringVault();
        MockLiquidTeller otherTeller = new MockLiquidTeller(otherLiquidAsset);
        otherLiquidAsset.setTeller(address(otherTeller));

        vm.prank(owner);
        module.addLiquidAssets(_single(address(otherLiquidAsset)), _single(address(otherTeller)));
        assertEq(address(module.liquidAssetToTeller(address(otherLiquidAsset))), address(otherTeller), "future route configured");

        uint256 amount = 25e6;
        otherAsset.mint(safeAddr, amount);
        ITradingSafeLiquidDepositModule.DepositRequest memory request = _request(safeAddr, address(otherAsset), address(otherLiquidAsset), amount, amount, DEADLINE);
        (address[] memory signers, bytes[] memory sigs) = _signRequestFor(address(module), request);

        vm.prank(relayer);
        module.depositToTopUp(request, signers, sigs);

        assertEq(otherAsset.balanceOf(safeAddr), 0, "new input deposited");
        assertEq(otherLiquidAsset.balanceOf(topUp), amount, "new vault shares forwarded");
    }

    function test_routeAdmin_removesVault() public {
        vm.prank(owner);
        module.removeLiquidAssets(_single(address(liquidBtc)));
        assertEq(address(module.liquidAssetToTeller(address(liquidBtc))), address(0), "route removed");

        wbtc.mint(safeAddr, WBTC_UNIT);
        (address[] memory signers, bytes[] memory sigs) = _sign(WBTC_UNIT, WBTC_UNIT, DEADLINE);
        vm.expectRevert(ITradingSafeLiquidDepositModule.UnsupportedLiquidAsset.selector);
        _deposit(WBTC_UNIT, WBTC_UNIT, DEADLINE, signers, sigs);
    }

    function test_routeAdmin_revertsForUnauthorizedCaller() public {
        vm.prank(makeAddr("random"));
        vm.expectRevert(ITradingSafeLiquidDepositModule.Unauthorized.selector);
        module.addLiquidAssets(_single(address(liquidBtc)), _single(address(teller)));
    ...
    function test_routeAdmin_revertsIfTellerVaultMismatch() public {
        MockBoringVault otherVault = new MockBoringVault();
        vm.prank(owner);
        vm.expectRevert(ITradingSafeLiquidDepositModule.InvalidConfiguration.selector);
        module.addLiquidAssets(_single(address(otherVault)), _single(address(teller)));
    }
    ...
    function test_deposit_mintsLiquidBtcAndForwardsToTopUp() public {
        ...
        emit ITradingSafeLiquidDepositModule.DepositedToTopUp(safeAddr, topUp, address(wbtc), address(liquidBtc), amount, amount);

        // Permissionless relay: an arbitrary caller submits the owner-signed authorization.
        vm.prank(relayer);
        _deposit(amount, amount, DEADLINE, signers, sigs);
    ...
    function test_deposit_revertsIfAmountZero() public {
        ...
        _deposit(0, 1, DEADLINE, signers, sigs);
    }
    ...
    function test_deposit_revertsIfMinReturnZero() public {
        ...
        _deposit(WBTC_UNIT, 0, DEADLINE, signers, sigs);
    }
    ...
    function test_deposit_revertsIfInsufficientBalance() public {
        ...
        _deposit(2 * WBTC_UNIT, 2 * WBTC_UNIT, DEADLINE, signers, sigs);
    }
    ...
    function test_deposit_revertsIfWbtcDebitInexact() public {
        ...
        _deposit(amount, 1, DEADLINE, signers, sigs);
    }
    ...
    function test_deposit_revertsIfSignatureOverDifferentAmount() public {
        ...
        _deposit(2 * WBTC_UNIT, WBTC_UNIT, DEADLINE, signers, sigs);
    }
    ...
    function test_deposit_revertsIfBelowThreshold() public {
        ...
        _deposit(WBTC_UNIT, WBTC_UNIT, DEADLINE, oneSigner, oneSig);
    }
    ...
    function test_deposit_revertsForNonTradingSafe() public {
        ...
        module.depositToTopUp(_request(notASafe, address(wbtc), address(liquidBtc), WBTC_UNIT, WBTC_UNIT, DEADLINE), signers, sigs);
    }

    // ---- Module enablement ----

    function test_deposit_revertsIfModuleNotEnabledOnSafe() public {
        TradingSafeLiquidDepositModule module2 = new TradingSafeLiquidDepositModule(_single(address(liquidBtc)), _single(address(teller)), address(dataProvider));
        address[] memory mods = new address[](1);
        ...
        module2.depositToTopUp(_request(safeAddr, address(wbtc), address(liquidBtc), WBTC_UNIT, WBTC_UNIT, DEADLINE), signers, sigs);
    }
    ...
    function _deposit(uint256 amount, uint256 minReturn, uint256 deadline, address[] memory signers, bytes[] memory sigs) internal {
        module.depositToTopUp(_request(safeAddr, address(wbtc), address(liquidBtc), amount, minReturn, deadline), signers, sigs);
    }
    ...
    function _signFor(address module_, uint256 amount, uint256 minReturn, uint256 deadline) internal view returns (address[] memory signers, bytes[] memory sigs) {
        return _signRequestFor(module_, _request(safeAddr, address(wbtc), address(liquidBtc), amount, minReturn, deadline));
    }

    function _signRequestFor(address module_, ITradingSafeLiquidDepositModule.DepositRequest memory request) internal view returns (address[] memory signers, bytes[] memory sigs) {
        bytes32 digest = keccak256(abi.encodePacked(keccak256("TradingSafeLiquidDepositModule.depositToTopUp"), block.chainid, module_, safe.nonce(), request.safe, topUp, keccak256(abi.encode(request)))).toEthSignedMessageHash();
    ...
    function _request(address safe_, address assetToDeposit, address liquidAsset, uint256 amount, uint256 minReturn, uint256 deadline) internal pure returns (ITradingSafeLiquidDepositModule.DepositRequest memory) {
        return ITradingSafeLiquidDepositModule.DepositRequest({ safe: safe_, assetToDeposit: assetToDeposit, liquidAsset: liquidAsset, amountToDeposit: amount, minReturn: minReturn, deadline: deadline });
    }
