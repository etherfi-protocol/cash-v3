...
contract AssetRecoveryDispatcherTest is Test {
    ...
    function setUp() public {
        ...
        address impl = address(new AssetRecoveryDispatcher(address(endpoint), OP_EID));
    ...
    function test_lzReceive_forwardsToTopUp() public {
        ...
        bytes memory message = RecoveryMessageLib.encode(RecoveryMessageLib.Payload({ safe: address(topup), token: address(token), recipient: recipient }));
    ...
    function test_lzReceive_revertsIfNotEndpoint() public {
        TopUpV2 topup = new TopUpV2(weth, address(dispatcher));
        bytes memory message = RecoveryMessageLib.encode(RecoveryMessageLib.Payload({ safe: address(topup), token: address(token), recipient: makeAddr("recipient") }));
    ...
    function test_lzReceive_revertsIfPayloadTargetsNonDispatcherTopUp() public {
        ...
        bytes memory message = RecoveryMessageLib.encode(RecoveryMessageLib.Payload({ safe: address(rogueTopup), token: address(token), recipient: makeAddr("recipient") }));
    ...
    function test_lzReceive_revertsIfTopUpNotDeployed() public {
        ...
        bytes memory message = RecoveryMessageLib.encode(RecoveryMessageLib.Payload({ safe: undeployed, token: address(token), recipient: makeAddr("recipient") }));
