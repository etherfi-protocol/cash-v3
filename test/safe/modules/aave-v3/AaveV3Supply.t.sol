...
contract AaveV3SupplyTest is AaveV3TestSetup {
    ...
    function test_supply_transfersTokensToPool() public {
        ...
        deal(address(usdc), address(safe), amountToSupply);

        bytes32 digestHash = keccak256(abi.encodePacked(aaveV3Module.SUPPLY_SIG(), block.chainid, address(aaveV3Module), aaveV3Module.getNonce(address(safe)), address(safe), abi.encode(address(usdc), amountToSupply))).toEthSignedMessageHash();
        ...
        emit AaveV3Module.SupplyOnAave(address(safe), address(usdc), amountToSupply);
    ...
    function test_supply_reverts_whenSignerIsNotAdmin() public {
        ...
        aaveV3Module.supply(address(safe), address(usdc), amountToSupply, notOwner, signature);
    }
    ...
    function test_supply_incrementsNonce() public {
        ...
        bytes32 digestHash = keccak256(abi.encodePacked(aaveV3Module.SUPPLY_SIG(), block.chainid, address(aaveV3Module), nonceBefore, address(safe), abi.encode(address(usdc), amountToSupply))).toEthSignedMessageHash();
    ...
    function test_supply_reverts_whenReplayingSignature() public {
        ...
        bytes32 digestHash = keccak256(abi.encodePacked(aaveV3Module.SUPPLY_SIG(), block.chainid, address(aaveV3Module), nonce, address(safe), abi.encode(address(usdc), amountToSupply))).toEthSignedMessageHash();
