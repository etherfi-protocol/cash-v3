...
contract FraxModuleTest is SafeTestSetup {
    ...
    function setUp() public override {
        ...
        fraxModule = new FraxModule(address(dataProvider), address(fraxusd), address(usdc), custodian, remoteHop);
    ...
    function test_deposit_successFraxUsd() public {
        ...
        bytes32 digestHash = keccak256(abi.encodePacked(fraxModule.DEPOSIT_SIG(), block.chainid, address(fraxModule), fraxModule.getNonce(address(safe)), address(safe), abi.encode(address(usdc), amountToDeposit))).toEthSignedMessageHash();
    ...
    function test_withdraw_successFraxUsd() public {
        ...
        bytes32 digestHash = keccak256(abi.encodePacked(fraxModule.WITHDRAW_SIG(), block.chainid, address(fraxModule), fraxModule.getNonce(address(safe)), address(safe), abi.encode(amountToWithdraw))).toEthSignedMessageHash();
    ...
    function test_AsyncWithdraw_success() public {
        vm.prank(owner);
        ...
        bytes32 depositAddress = bytes32(uint256(uint160(address(0xBdeb781661142740328fDefc1D9ecd03778fd810))));

        bytes32 digestHash = keccak256(abi.encodePacked(fraxModule.ASYNC_WITHDRAW_SIG(), block.chainid, address(fraxModule), fraxModule.getNonce(address(safe)), safe, abi.encode(fraxusd, depositAddress, amountToWithdraw))).toEthSignedMessageHash();
        ...
        vm.expectEmit(true, true, true, true);
        emit FraxModule.AsyncWithdrawal(address(safe), amountToWithdraw);

        fraxModule.asyncWithdraw(address(safe), depositAddress, amountToWithdraw, fee.nativeFee, owner1, signature);
    ...
    //Revert cases
    //Todo: add async withdraw revert test cases
