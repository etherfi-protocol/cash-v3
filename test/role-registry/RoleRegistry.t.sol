...
contract RoleRegistryTest is Test {
    ...
    function setUp() public {
        ...
        vm.mockCall(
            dataProviderMock,
            abi.encodeWithSelector(IEtherFiDataProvider.onlyEtherFiSafe.selector, safe),
            abi.encode()
        );
        ...
        vm.mockCallRevert(
            dataProviderMock,
            abi.encodeWithSelector(IEtherFiDataProvider.onlyEtherFiSafe.selector, nonAdmin),
            abi.encodeWithSelector(EtherFiDataProvider.OnlyEtherFiSafe.selector)
        );
    ...
    function test_getSafeAdmins_returnsAllAdmins() public {
        ...
        assertTrue(
            (admins[0] == safeAdmin && admins[1] == admin) || 
            (admins[0] == admin && admins[1] == safeAdmin),
            "Should return all added admins"
        );
    }
