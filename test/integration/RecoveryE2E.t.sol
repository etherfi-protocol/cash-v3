...
contract RecoveryE2ETest is SafeTestSetup {
    ...
    function test_e2e_opToArb_happyPath() public {
        ...
        // Plant a controlled TopUpFactory in the Solady owner slot and stub
        // `isTokenSupported` to false so `executeRecovery`'s unsupported-token gate is happy
        // (in prod the owner is the local TopUpFactory, set by `TopUpFactory.deployTopUpContract`).
        bytes32 ownerSlot = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffff74873927;
        address fakeFactory = makeAddr("topUpFactory");
        vm.store(address(safe), ownerSlot, bytes32(uint256(uint160(fakeFactory))));
        vm.mockCall(
            fakeFactory,
        ...
        assertEq(token.balanceOf(address(safe)), 0, "topup should be drained");
    }
