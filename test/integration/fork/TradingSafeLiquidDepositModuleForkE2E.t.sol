...
contract TradingSafeLiquidDepositModuleForkE2E is Test {
    ...
    function setUp() public {
        ...
        module = new TradingSafeLiquidDepositModule(DATA_PROVIDER, WBTC, LIQUID_BTC, LIQUID_BTC_TELLER);
    ...
    function test_fork_depositWbtcToLiquidBtcLandsAtTopUpAndSweeps() public {
        ...
        (address[] memory signers, bytes[] memory sigs) = _sign(DEPOSIT_AMOUNT, 1, DEADLINE);

        uint256 topUpBefore = IERC20(LIQUID_BTC).balanceOf(topUp);

        // Permissionless relay submits the owner-signed authorization.
        vm.prank(relayer);
        module.depositToTopUp(safeAddr, DEPOSIT_AMOUNT, 1, DEADLINE, signers, sigs);
    ...
    function _sign(uint256 amount, uint256 minReturn, uint256 deadline) internal view returns (address[] memory signers, bytes[] memory sigs) {
        bytes32 digest = keccak256(abi.encodePacked(keccak256("TradingSafeLiquidDepositModule.depositToTopUp"), block.chainid, address(module), safe.nonce(), safeAddr, topUp, amount, minReturn, deadline)).toEthSignedMessageHash();
