 ...
 */
contract EtherFiLiquidModuleWithReferrer is ModuleBase, ModuleCheckBalance, ReentrancyGuardTransient, IBridgeModule, ModuleLendGatewaySandwich {
    ...
    function _deposit(address safe, address assetToDeposit, address liquidAsset, uint256 amountToDeposit, uint256 minReturn) internal {
        ...
        if (assetToDeposit == ETH) {
            ...
            to[2] = address(teller);
            data[2] = abi.encodeWithSelector(ILayerZeroTellerWithReferrer.deposit.selector, ERC20(weth), amountToDeposit, minReturn, address(0));
        } else {
            ...
            to[1] = address(teller);
            data[1] = abi.encodeWithSelector(ILayerZeroTellerWithReferrer.deposit.selector, ERC20(assetToDeposit), amountToDeposit, minReturn, address(0));
        }
