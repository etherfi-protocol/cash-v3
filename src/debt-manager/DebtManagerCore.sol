...
contract DebtManagerCore is DebtManagerStorage {
    ...
    function totalBorrowingAmounts() public view returns (TokenData[] memory, uint256) {
        ...
        for (uint256 i = 0; i < len;) {
            BorrowTokenConfig memory config = borrowTokenConfig(_supportedBorrowTokens[i]);

            if (config.totalBorrowingAmount > 0) {
                tokenData[m] = TokenData({ token: _supportedBorrowTokens[i], amount: config.totalBorrowingAmount });
    ...
    function getMaxBorrowAmount(address user, bool forLtv) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < len;) {
            ...
            if (forLtv) {
                ...
                totalMaxBorrow += collateral.mulDiv(_collateralTokenConfig[collateralTokens[i].token].ltv, HUNDRED_PERCENT, Math.Rounding.Floor);
            } else {
                totalMaxBorrow += collateral.mulDiv(_collateralTokenConfig[collateralTokens[i].token].liquidationThreshold, HUNDRED_PERCENT, Math.Rounding.Floor);
            }
    ...
    function getBorrowingPowerAndTotalBorrowing(address user, TokenData[] memory tokenAmounts) external view returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < len;) {
            ...
            totalMaxBorrow += collateral.mulDiv(_collateralTokenConfig[tokenAmounts[i].token].ltv, HUNDRED_PERCENT, Math.Rounding.Floor);
    ...
    function supplierBalance(address supplier, address borrowToken) public view returns (uint256) {
        ...
        return _sharesOfBorrowTokens[supplier][borrowToken].mulDiv(_getTotalBorrowTokenAmount(borrowToken), _borrowTokenConfig[borrowToken].totalSharesOfBorrowTokens, Math.Rounding.Floor);
    }
    ...
    function withdrawBorrowToken(address borrowToken, uint256 amount) external {
        ...
        uint256 shares = amount.mulDiv(_borrowTokenConfig[borrowToken].totalSharesOfBorrowTokens, totalBorrowTokenAmt, Math.Rounding.Ceil);
    ...
    function _liquidStableAmounts() internal view returns (TokenData[] memory) {
        ...
        for (uint256 i = 0; i < len;) {
            uint256 bal = IERC20(_supportedBorrowTokens[i]).balanceOf(address(this));

            if (bal > 0) {
                tokenData[m] = TokenData({ token: _supportedBorrowTokens[i], amount: bal });
