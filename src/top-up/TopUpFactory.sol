...
contract TopUpFactory is BeaconFactory {
    ...
    function bridge(address token) external payable whenNotPaused {
        ...
        _delegateCall($.tokenConfig[token].bridgeAdapter, abi.encodeWithSelector(BridgeAdapterBase.bridge.selector, token, balance, $.tokenConfig[token].recipientOnDestChain, $.tokenConfig[token].maxSlippageInBps, $.tokenConfig[token].additionalData));
