...
contract TopUpDest is UpgradeableProxy {
     ...
     *      msg.sender guard keeps it a private helper. No-op unless the safe is on the lend gateway
     *      with lend enabled and the token is registered.
    ...
    function supplyTopUpToLend(address safe, address token, uint256 amount) external {
        ...
        if (!cashModule.usesLendGateway(safe) || !cashModule.isLendEnabled(safe)) return;
