 ...
 * @author ether.fi
 */
contract TopUpDestWithMarkCompleteAdmin is UpgradeableProxy {
    ...
    function markCompletedAdmin(bytes32[] memory txHashes, address[] memory users, uint256[] memory chainIds, address[] memory tokens, uint256[] memory amounts) external onlyRoleRegistryOwner {
        uint256 len = txHashes.length;
        if (len != users.length ||  len != tokens.length) revert ArrayLengthMismatch();
