...
contract SettlementDispatcherV2 is UpgradeableProxy, Constants {
     ...
     * @custom:throws StargateValueInvalid If the Stargate router doesn't support the token
     */
    function setDestinationData(address[] calldata tokens, DestinationData[] calldata destDatas) external virtual onlyGovernanceMultisig {
     ...
     */
    function setLiquidAssetWithdrawQueue(address asset, address boringQueue) external onlyGovernanceMultisig {
     ...
     */
    function setRefundWallet(address _refundWallet) external onlyGovernanceMultisig {
     ...
     */
    function setFraxConfig(address _fraxUsd, address _fraxCustodian, address _fraxRemoteHop, address _fraxAsyncRedeemRecipient) external onlyGovernanceMultisig {
     ...
     */
    function setMidasRedemptionVault(address midasToken, address redemptionVault) external onlyGovernanceMultisig {
     ...
     * @custom:throws WithdrawFundsFailed If ETH transfer fails
     */
    function withdrawFunds(address token, address recipient, uint256 amount) external nonReentrant onlyGovernanceMultisig() {
     ...
     */
    function setCCTPConfig(address _tokenMessenger, uint32 _destinationDomain, uint256 _maxFee, uint32 _minFinalityThreshold) external onlyGovernanceMultisig {
     ...
     * @dev Only callable by the governance multisig (GOVERNANCE_ROLE). Each token can have a different recipient.
     * @param tokens Array of token addresses to configure
     * @param recipients Array of recipient addresses corresponding to each token
     */
    function setSettlementRecipients(address[] calldata tokens, address[] calldata recipients) external onlyGovernanceMultisig {
